...
total_mutations <- rbind(left_join(total_mutations %>% filter(host_sex=="male"), male_table),
      left_join(total_mutations %>% filter(host_sex=="female"), female_table)) %>%
  mutate(normalized = round(n / sample_count,0))

# this gives a column n for the number of gene by position
ggplot(total_mutations, aes(x = Municipalities, y = normalized, fill = host_sex)) +
    geom_bar(stat='identity', position='dodge') + # makes a bar plot
