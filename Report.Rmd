...
* 1. Retrieve the SARS-CoV-2 Sequencing Data (SraRunTable): 
The metadata for Brazil was downloaded from the NCBI website.
* 2. Download the raw sequences and check the quality of data:
The downloaded raw sequences data was pushed through 'fasterq-dump', a program written 
by NCBI to make the downoading of raw sequences feasible. The data was forked by
'fastqc' and analysed for its quality.
* 3. Eliminate low quality and unnecessary data:
The raw sequencing dataset contains number of redundant data. To trim off the primers, delete low-quality base pairs and bad reads, 'trimmomatic' is used.
* 4. Sorting the dataset:
The trimmed data is mapped against the reference genome and index were created using 'bwa'. The data was sorted with the help of sam_and_bam tools.
* 5. Data visualization and pearl script:
To visualize the processed and mapped data, it is passed through pearl scripting. 
'vcf' and 'igv' are used to view reads.
* 6. Graphical analysis:
I have used ggplots, ggthemes to create plots, geome, scales of my data to understand more about 
the sequences.
* 7. Analysing visuals and knitting:
Using vcfR the data is passed to 'R' for visualization and knitted using 'knitter'
to create the report of the analysis. 

SraRuntable: Using SraRunTable, I downloaded SRA Run table from the internet.
fasterq-dump: It was input to the program 'fasterq-dump' written by NCBI to download raw sequence data to the computer in a fast way.
fastqc: I used forked 'fastqc' to check the quality of raw data.
trimmomatic: The data was sent to 'trimmomatic' which is an adapter to trim off primers and also gets rid of low quality bases and bad reads.
bwa (Burrows-Wheeler Aligner): The downloaded, cleaned and trimmed data was fed into 'bwa' (Burrows-Wheeler Aligner) that created the index and maps the short reads against the reference genome.
sam_and_bam: Later the data goes to sam_and_bam files i.e. samtools and bamtools which sorted and processed the mapped reads. 
vcf files: Then it was passed to pearl script which takes mapped and processed sam_and_bam files and processes it into 'vcf' files.
igvR: The mapped and processed reads can be viewed in IGV.
vcfR[@knaus2017vcfr]: It is passed to 'R' using the package 'vcfR' for the analysis and visualization.
...
gene_table %>%
  ...
  knitr::kable(col.names = c("Gene Name", "Sequence Start" ,
               ...
               align = 'c',
               caption = "Table 1: An example table caption it is long.",
               booktabs = TRUE) %>%
